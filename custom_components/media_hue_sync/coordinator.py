...
class SonosHueCoordinator:
    ...
    async def async_setup(self):
        _LOGGER.info("Setting up Media Hue Sync: sonos=%s lights=%s", self.sonos_entity, self.light_entities)
    ...
    async def async_show_help(self):
        message = """## Media Hue Sync Help & Guide

### What it does
Media Hue Sync watches the selected Sonos player, extracts colors from the current album art, applies those colors to Hue lights, and restores the previous lighting state when playback stops or Sync is turned off.
...
**Settings → Devices & services → Media Hue Sync → three-dot menu → Download diagnostics**
...
"""
    ...
    async def _restore_snapshot_now(self, reason="restore", clear_snapshot=True):
        ...
        try:
            ...
            _LOGGER.exception("Failed restoring Media Hue Sync scene")
        clear_apply_cache()
        self._notify()
        return self.last_restore_result
    ...
    async def async_health_check(self) -> dict:
        ...
        await self.hass.services.async_call(
            ...
            "create",
            {
                "title": "Media Hue Sync Health Check",
    ...
    async def _capture_restore_snapshot(self, reason="snapshot"):
        """Capture target light state before Media Hue Sync changes anything."""
