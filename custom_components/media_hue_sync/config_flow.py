...
from .const import (
    ...
    DEFAULT_GRADIENT_NEUTRAL_SUPPRESSION,
...
def _full_schema(defaults: dict):
    ...
    return {
        ...
        vol.Optional(CONF_GRADIENT_NEUTRAL_SUPPRESSION, default=defaults.get(CONF_GRADIENT_NEUTRAL_SUPPRESSION, DEFAULT_GRADIENT_NEUTRAL_SUPPRESSION)):
            selector.SelectSelector(selector.SelectSelectorConfig(options=_select_options(GRADIENT_NEUTRAL_SUPPRESSION_OPTIONS, GRADIENT_NEUTRAL_SUPPRESSION_LABELS), mode=selector.SelectSelectorMode.LIST)),
...
class SonosHueConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        if user_input is not None:
            return self.async_create_entry(title="Media Hue Sync", data=user_input)
